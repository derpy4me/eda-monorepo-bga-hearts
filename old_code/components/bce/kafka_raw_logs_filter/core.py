...
class BgaLogFilter:
    ...
    def get_game_id_from_raw_log(self, log_message: dict[str, Any]) -> str | None:
        channel = log_message.get("push", {}).get("channel")
        if not channel:
            return None
        match = re.search(r"/table/([a-zA-Z0-9]+)", channel)
    ...
    def filter_message(self, log_message: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        Example:
            ...
            ...     process_event(event)
        """
        game_id = self.get_game_id_from_raw_log(log_message)
        if not game_id:
            return []
        relevant_events = []
